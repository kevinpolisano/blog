---
title: "Quand interrompre un jeu devient un problème mathématique"
author: "Kévin Polisano"
date: "2026-03-20"
image: karpov-kasparov.jpg
...
*Cet article a été écrit dans le cadre de la [Semaine des mathématiques 2026](https://www.education.gouv.fr/la-semaine-des-mathematiques-465828) ﻿ sur le thème **"Égalités"**, et propose d'aborder les principes de **justice** et **d'équité** sous l'angle des mathématiques.*

# Un championnat du monde des échecs interrompu

En 1984, le monde des échecs assiste à un duel d'anthologie entre [Anatoli Karpov](https://fr.wikipedia.org/wiki/Anatoli_Karpov), champion en titre, et le jeune prodige [Garry Kasparov](https://fr.wikipedia.org/wiki/Garry_Kasparov). Le match est atypique : il ne se joue pas en un nombre fixé de parties, mais jusqu’à ce que l’un des joueurs atteigne six victoires. Les nulles n'étant pas comptabilisées. Karpov mène rapidement 4-0 après seulement 9 parties, puis 5-0 après la 27 ème. Il est à un point de la victoire.  Kasparov s'accroche, et finit par marquer un point à  la 32ème partie. S'en suit une longue série de nulles, puis, contre toute attente, Kasparov renverse la vapeur et remporte deux parties consécutives. Après [48 parties](https://fr.wikipedia.org/wiki/Championnat_du_monde_d%27%C3%A9checs_1984)﻿ — un record — le score est de 5 victoires à 3 pour Karpov, avec 40 nulles au total !

![Anatoli Karpov (à gauche) et Garry Kasparov (à droite)](karpov-kasparov.jpg)

La tension extrême, et après plus de 5 mois d'affrontement, l'épuisement physique se fait ressentir (Karpov a perdu 10 kg depuis le début du championnat). La FIDE décide d’interrompre le match sans déclarer de vainqueur, pour des raisons officiellement liées à la santé des joueurs, mais probablement aussi pour des raisons politiques... La récompense s'élevait à [1,9 million](https://www.sjakknyheter.no/champions/world-chess-championship-1984/)﻿ de francs suisses. Cependant le match n'a pas été mené à terme. Une question naturelle se pose alors : comment répartir les gains d’un match interrompu ? Mathématiquement, cette question est loin d’être anodine : elle renvoie à un problème vieux de plusieurs siècles, connu sous le nom de **problème des partis**, au sens de la juste répartition, et dont la solution requiert un **partage équitable**.

# Le problème des partis

[Luca Pacioli](https://fr.wikipedia.org/wiki/Luca_Pacioli), moine franciscain et ami de [Léonard de Vinci](https://fr.wikipedia.org/wiki/L%C3%A9onard_de_Vinci), expose dès 1494 le problème des partis dans un [traité d’arithmétique](https://fr.wikipedia.org/wiki/Summa_de_arithmetica,_geometria,_de_proportioni_et_de_proportionalita) [@pacioli1494suma], à travers l’exemple suivant :
...
Cette solution ne convainc pas [Niccolò Tartaglia](https://fr.wikipedia.org/wiki/Niccol%C3%B2_Fontana_Tartaglia), qui écrit en 1556 [@tartaglia1556general] :
...
- Tartaglia propose une autre idée : **faire dépendre l’écart de gain de l’écart de points** [@tartaglia1556general]. Ici, l’écart est de 30 points, soit la moitié des 60 nécessaires pour gagner. Il suggère donc que l’écart de gain soit la moitié de la mise, soit 5 ducats. Le premier reçoit alors 7,5 ducats et le second 2,5.

- [Lorenzo Forestani](https://it.wikipedia.org/wiki/Lorenzo_Forestani) propose en 1603 de **tenir compte de la durée maximale du jeu** [@forestani1682pratica]. Le jeu dure au plus 11 coups (scénario 60 contre 50). Ici, 7 coups ont déjà été joués, il en reste donc 4, qu’il propose de partager équitablement. Le premier reçoit ainsi $\frac{5+2}{11}$ de la mise (soit 6,36 ducats) et le second $\frac{2+2}{11}$ (soit 3,64 ducats).

- En 1539, [Gerolamo Cardano](https://fr.wikipedia.org/wiki/J%C3%A9r%C3%B4me_Cardan) (en français Jérôme Cardan) plaide  pour une **répartition de l'enjeu selon les points qu'il reste à marquer** [@cardano1539practica]. Le premier n’a plus qu’un point à obtenir, contre quatre pour le second. En utilisant des progressions arithmétiques (1 d’un côté, $1+2+3+4=10$ de l’autre), il propose une répartition en $\frac{10}{11}$ et $\frac{1}{11}$, soit environ 9,1 ducats pour le premier et 0,9 pour le second.
...
En 1654, [Antoine Gombaud](https://en.wikipedia.org/wiki/Antoine_Gombaud), connu sous le nom de chevalier de Méré, soumit le problème à [Blaise Pascal](https://en.wikipedia.org/wiki/Blaise_Pascal), qui entama alors une [correspondance](https://gallica.bnf.fr/ark:/12148/bpt6k69975r/f204.image) avec [Pierre de Fermat](https://en.wikipedia.org/wiki/Pierre_de_Fermat). De cette discussion entre ces deux géants des mathématiques émergèrent non seulement une solution rigoureuse et satisfaisante, mais aussi les premières idées qui allaient donner naissance à la théorie des probabilités.
...
Pascal et Fermat lancent une pièce à tour de rôle : si elle tombe sur pile (P), Pascal remporte la manche, tandis que si elle tombe sur face (F), Fermat la remporte. La victoire est accordée au premier qui remporte trois manches. Pascal et Fermat **misent une somme égale d’argent**, disons 32€, et le vainqueur récupère l’ensemble de l’argent mis en jeu, soit 64€.

Pour une raison quelconque, le jeu est interrompu sans qu'aucun des joueurs n'ait remporté trois manches. Quelle doit être la juste répartition des mises initiales, se demande Pascal, de sorte que celle-ci soit établie *« proportionnellement à ce qu’ils sont en droit d’espérer de la fortune »* ?
...
Le schéma animé ci-dessous illustrant le procédé itératif, ainsi que le suivant, est inspiré de l'[exposé](https://www.youtube.com/watch?v=IRGQkzm-gyw) donné par [Grégory Miermont](https://fr.wikipedia.org/wiki/Gr%C3%A9gory_Miermont) et adapté de [@mathieu2023blaise].

![](animation-pascal.gif)

Dans le cas où la partie aurait été interrompue après un seul lancer tombé sur pile, où il manque donc deux parties à gagner pour Pascal pour Fermat, on en conclut que Pascal doit recevoir $\color{violet}E(2, 3)=44$ euros et Fermat 20€.
...
On dénombre ainsi [11 cas favorables]{style="color:blue;"} à Pascal sur 16,﻿ donc il gagne avec probabilité $\color{blue}\frac{11}{16}$ et empoche $$\color{violet} E(2,3)=64 \times {\color{blue} \frac{11}{16}} = 44$$ euros comme déjà calculé par la méthode de Pascal.
...
Les [coefficients binomiaux](https://fr.wikipedia.org/wiki/Coefficient_binomial)﻿ $\binom{n}{k}$, appelés aussi combinaisons, représentent le nombre de façons de sélectionner $k$ objets parmi une collection de $n$ objets distincts. Puisque l'on a la relation ${n \choose k} + {n \choose k+1} = {n+1 \choose k+1}$ et ${n \choose 0}=1$, les coefficients binomiaux se calculent pas à pas et forment le [triangle de Pascal](https://fr.wikipedia.org/wiki/Triangle_de_Pascal) (qui n'est pas sans rappeler la méthode par récurrence exposée précédemment).
...
Fermat raisonne donc à partir de situations que l’on peut qualifier de *« fictives »*. Blaise Pascal en est parfaitement conscient, et il en défend la pertinence (du moins pour deux joueurs) : ces manches feintes n’altèrent en rien le résultat final. Elles permettent simplement de mettre toutes les issues sur un pied d’égalité et de simplifier le calcul. Les mathématiques ont l’art d’ajouter des éléments imaginaires pour pouvoir résoudre plus facilement certains problèmes (les nombres complexes, appelés aussi *nombres imaginaires*, en sont une parfaite illustration). Comme le souligne Pierre de Fermat lui-même : *« cette fiction d’étendre le jeu à un certain nombre de parties, ne sert qu’à faciliter la règle, et (suivant mon sentiment) à rendre tous les hasards égaux »*. Il précise encore que *« l’extension à un certain nombre de parties n’est autre chose que la réduction de diverses fractions à un même dénominateur »* et conclut, dans un esprit de conciliation à l’égard de Pascal : *« Voilà en peu de mots tout le mystère, qui nous remettra sans doute en bonne intelligence, puisque nous ne cherchons l’un et l’autre que la raison et la vérité »*.

Pour conclure avec [@mathieu2023blaise] :

> L’un des grands intérêts de la résolution de ce problème dans la correspondance entre Pascal et Fermat est que leurs deux solutions sont très différentes. Elles parviennent pourtant au même résultat, ce qui nous vaudra le joli mot de Pascal : « je vois bien que la vérité est la même à Toulouse et à Paris ». En langage moderne, on dirait que Fermat trouve un bon espace de probabilités dans lequel le problème se ramène à une simple question de dénombrement. De son côté, Pascal voit le jeu comme une sorte de contrat passé entre les joueurs, et s’intéresse toujours à ce que chacun peut revendiquer sur une somme engagée dans le jeu. Ce faisant, il invente la notion d’espérance mathématique.

Cette notion d’[espérance mathématique](https://fr.wikipedia.org/wiki/Esp%C3%A9rance_math%C3%A9matique) sera ensuite formalisée par [Christian Huygens](https://fr.wikipedia.org/wiki/Christian_Huygens) dans son traité sur le calcul des jeux de hasard [@huygens1657ratiociniis].
