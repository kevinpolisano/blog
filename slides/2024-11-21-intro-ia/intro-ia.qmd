...
render_media <- function(version, local_src = NULL, youtube_src = NULL, ratio = 1, margin = 0) {
  if (version == "local") {
    ...
    return(knitr::asis_output(paste0(
      ...
      '<iframe data-external="1" data-autoplay="1" width="', width,'" height="', height, '" src="', youtube_src, '&amp;loop=1&amp;controls=0&amp;modestbranding=1&amp;autohide=1"',
...
### 1943 - Warren McCulloch et Walter Pitts {.colored-title}
...
### 1957 - Frank Rosenblatt {.colored-title}
...
## L'invention du Perceptron Multicouche {background-color="black"}
### 1986 - Geoffrey Hinton {.colored-title}
...
## Que se passe-t-il dans un réseau de neurones ? {background-color="black"}
...
## Pourquoi les réseaux de neurones fonctionnent si bien ?
...
### Fonctions continues approchables par un réseau à 2 couches

::: {.callout-note}
## Théorème (Cybenko, 1989, Hornik, 1991)
Let $\sigma:\mathbb{R}\mapsto \mathbb{R}$ be a nonconstant, bounded, and continuous function. Let $I_m$ denote the $m$-dimensional unit hypercube $[0, 1]^m$. The space of real-valued continuous functions on $I_m$ is denoted by $C(I_m)$. Then, given any $\epsilon > 0$ and any function $f\in C(I_m)$, there exist an integer $N$, real constants $v_i, b_i\in \mathbb{R}$ and real vectors $\boldsymbol{w}_i \in \mathbb{R}^m$ for $i = 1, \dotsc, N$, such that we may define:

$$ F(\boldsymbol{x}) = \sum_{i=1}^N v_i \sigma\left(\boldsymbol{w}_i^T \boldsymbol{x}+b_i\right)=\boldsymbol{v}^T \sigma\left(\mathbf{W}^T \boldsymbol{x}+\boldsymbol{b}\right)$$
as an approximate realization of the function $f$; that is,
$$ |f(\boldsymbol{x})-F(\boldsymbol{x})|<\epsilon, \quad \forall \boldsymbol{x}\in I_m$$
::: 
